...
public class UserInfoDomainService {
	...
	public UserInfo getUserInfo(String name) {
		...
		List<UserInfo> lstUserInfo = userInfoRepository.findAll();
		
		Optional<UserInfo> userInfo = null;
		
		for(UserInfo chkUserInfo : lstUserInfo ) {
			if(chkUserInfo.getName().equals(name)) {
				userInfo = Optional.ofNullable(chkUserInfo);
				break;
			}
		}
